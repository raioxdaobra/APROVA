...
const multiTopicSchema = z.object({
  topics: z
    ...
    .min(1)
    .max(120),
  /** Quantidade alvo de questões. Default = SIMULADO_MAX (pool inteiro até teto). */
  total: z.number().int().min(5).max(120).optional(),
...
export async function startMultiTopicSimuladoAndRedirect(
  ...
  const requestedTotal = parsed.data.total ?? null;
  ...
  // Respeita o total escolhido pelo user, capado no SIMULADO_MAX e no pool disponível.
  const targetTotal = Math.min(
    requestedTotal ?? SIMULADO_MAX,
    SIMULADO_MAX,
    pool.length,
  );
  const questionIds = shuffle(pool).slice(0, targetTotal).map((q) => q.id);
